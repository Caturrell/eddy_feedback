...
and then copied from: /home/users/cturrell/documents/eddy_feedback/chapter1/cmip6/daily-efp_cmip6/calc_daily-efp_annual_cycle.py
...
if __name__ == "__main__":
    ...
    ### Calculate (or check for) monthly means for all models (if not already done) ###
    logger.info("\n" + "="*70)
    logger.info("STEP 1: Monthly Resampling")
    logger.info("="*70)
    
    # Create save path for monthly data
    save_mon_path = Path(cmip_path) / 'mon_avg_daily'
    save_mon_path.mkdir(parents=True, exist_ok=True)
    
    # Get list of models
    daily_avg_path = Path(cmip_path) / 'daily_averages'
    ...
    for idx, model_dir in enumerate(model_list, 1):
        ...
        save_file_name = f'{model_name}_1950_2014_mm_uvt_ubar_epf.nc'
