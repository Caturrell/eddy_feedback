...
YEARS = (1979, 2014)   # for title and filename; also used to load reanalysis data
...
def load_cmip6(time_freq):
    path = f'/home/links/ct715/eddy_feedback/chapter1/cmip6/historical_runs/data/{YEARS[0]}_{YEARS[1]}/cmip6_hist_{YEARS[0]}_{YEARS[1]}_{time_freq}_efp_winters.csv'
    df = pd.read_csv(path)
...
if COMBINED_FIGURE:
    output_path = f'/home/links/ct715/eddy_feedback/chapter1/cmip6/historical_runs/analysis_historical/plots/hist_6h+daily_spatial-scales_{YEARS[0]}-{YEARS[1]}.png'
    ...
    for ax, time_freq in zip(axes, TIME_FREQS):
        ...
        ax.set_title(f"CMIP6 Historical Winter EFP Spread ({time_freq})", fontsize=16, pad=15)
    ...
    for time_freq in TIME_FREQS:
        print(f"\n{'─' * 60}\nProcessing: {time_freq}\n{'─' * 60}")
        output_path  = f'/home/links/ct715/eddy_feedback/chapter1/cmip6/historical_runs/analysis_historical/plots/hist_{time_freq}_spatial-scales_{YEARS[0]}-{YEARS[1]}.png'
        df_long      = load_cmip6(time_freq)
        bootstrap_df = load_bootstrap(time_freq)
