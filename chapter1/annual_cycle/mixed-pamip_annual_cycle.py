...
def compute_and_save_efp_seasonal(dataset, output_dir):
    ...
    """
        Slightly different function because it opens and saves the results to a JSON file.
    """
    ...
    configs = [
        ('efp_nh_total_mon', 'divFy', False),
        ('efp_sh_total_mon', 'divFy', True),
    ]
    ...
    for key, which_div1, calc_south_hemis in configs:
        ...
        for season, months in season_month_dict.items():
            ...
            result[season] = {
                "efp": float(efp),  # ensure it’s JSON-serializable
                "months": months
            }
...
if __name__ == "__main__":
    ...
    pamip_data_path = '/home/links/ct715/data_storage/PAMIP/processed_monthly/efp_pd_non-regridded'
    ...
    for model in models:
        ...
        if model == 'AWI-CM-1-1-MR':
            logging.info("Skipping AWI-CM-1-1-MR model as it is not available with this data.")
            continue
        
        # import daily data
        daily_path = os.path.join(pamip_data_path, f'{model}_*.nc')
        daily = xr.open_mfdataset(daily_path)
        
        datasets[model] = daily
