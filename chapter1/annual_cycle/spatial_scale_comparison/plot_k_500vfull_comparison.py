...
data = {
    'mean': {'6h':    load_mean('6hourly_efp'),
             'daily': load_mean('daily_efp')},
    '500':  {'6h':    load_500('500hPa_6hourly_efp'),
             'daily': load_500('500hPa_daily_efp')},
}
...
# ── Figure 1: 2×2 — rows: NH/SH | cols: mean/500 hPa ─────────────────────────
...
for row, (hemi, all_key, k123_key, gt3_key) in enumerate(row_cfg):
    for col, (level_label, level_key, label_t, label_b) in enumerate(col_cfg):
        ax = axes[row, col]
        ...
        for freq in ('6h', 'daily'):
            ...
            xs, ys = extract_series(level_data[freq], all_key)
            ax.plot(xs, ys, marker='o', linewidth=2, color=colors['all'], linestyle=ls)

            xs, ys = extract_series(level_data[freq], k123_key)
            ax.plot(xs, ys, marker='o', linewidth=2, color=colors['123'], linestyle=ls)

            xs, ys = extract_series(level_data[freq], gt3_key)
        ...
        ax.set_title(f'$\\bf{{({panel_label})}}$ {hemi} — {level_label}', fontsize=13)
...
plt.suptitle(f'500 hPa vs mean-level EFP by spatial scale ({TS.replace("_", "–")})',
             fontsize=14, y=1.01)
plt.tight_layout(rect=[0, 0.06, 1, 1])

PLOTS = '/home/links/ct715/eddy_feedback/chapter1/annual_cycle/spatial_scale_comparison/plots'
os.makedirs(PLOTS, exist_ok=True)
save_path = os.path.join(PLOTS, '500vfull_comparison.png')
plt.savefig(save_path, dpi=150, bbox_inches='tight')
print(f"Saved: {save_path}")


# ── Figure 2: 2×3 — rows: frequency | cols: k-group ──────────────────────────
# colours: NH vs SH | linestyles: mean vs 500 hPa
...
for row, col, nh_key, sh_key, col_title, panel_label in panel_cfg:
    ...
    for level_key, ls in (('mean', '-'), ('500', '--')):
        d = data[level_key][freq]
...
legend2_handles = [
    ...
    Line2D([0], [0], color='k', lw=2, linestyle='-', label='Mean level'),
    Line2D([0], [0], color='k', lw=2, linestyle='--',  label='500 hPa'),
]
fig2.legend(handles=legend2_handles, loc='lower center', ncol=4,
            bbox_to_anchor=(0.5, -0.02), frameon=True, fontsize=12)

plt.suptitle(f'500 hPa vs mean-level by spatial scale and frequency ({TS.replace("_", "–")})',
             fontsize=14, y=1.01)
plt.tight_layout(rect=[0, 0.06, 1, 1])

save_path2 = os.path.join(PLOTS, '500vfull_comparison_6panel.png')
