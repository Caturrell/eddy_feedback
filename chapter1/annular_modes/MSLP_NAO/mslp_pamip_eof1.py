...
# =============================================================================
data_dir = '/home/links/ct715/data_storage/PAMIP/processed_monthly/mslp'
path_dir = Path(data_dir)
models = [m.name for m in sorted(path_dir.iterdir())]

# Load all datasets and assign a model attribute for later use
ds = {}
for model in models:
    model_path = path_dir / model
    files = sorted(model_path.glob('*.nc'))
    ds[model] = xr.open_mfdataset(files, combine='nested', concat_dim='ens_ax').assign_attrs(model=model)
...
def compute_anomalies(ds_model, season, months):
    """
    ...
    u_ens = seasonal_avg['psl']
    # Compute ensemble anomalies and select the desired region.
    var_anoms = u_ens - u_ens.mean(dim='ens_ax')
    var_anoms = var_anoms.sel(lat=slice(0,90)).rename({'ens_ax': 'time'})
    
    return var_anoms

def get_or_compute_eof(model, season, months, save_file):
    """
    ...
    if os.path.isfile(save_file):
        ...
        var_anoms = compute_anomalies(ds_model, season, months)
...
save_data_path = '/home/links/ct715/eddy_feedback/chapter1/annular_modes/MSLP_NAO/data/pamip_mslp'
os.makedirs(save_data_path, exist_ok=True)
plot_base_path = '/home/links/ct715/eddy_feedback/chapter1/annular_modes/MSLP_NAO/plots/pamip_eof1s_mslp'
...
for season, months in seasons.items():
    ...
    # -------------------------------------------------------------------------
    # Figure 1: Facet Grid of Full Resolution EOF Fields (Polar Projection - NH)
    # -------------------------------------------------------------------------
    results = {}
    for model in models:
        file_name = f'{model}_eofs_{season}.nc'
        data_file = os.path.join(save_data_path, file_name)
        results[model] = get_or_compute_eof(model, season, months, data_file)
    ...
    for i, model in enumerate(models):
        ax = plt.subplot(rows_grid, cols, i + 1, projection=ccrs.NorthPolarStereo())
    ...
    fig.suptitle(f"EOF 1 of MSLP ({season}, NH)", fontsize=14)
    fig.tight_layout(rect=[0, 0.1, 1, 0.95])

    facet_plot_file = os.path.join(plot_save_path, f"facet_eof1_{season}_NH.pdf")
    fig.savefig(facet_plot_file)
    print(f"Facet grid plot for {season} (NH) saved to {facet_plot_file}")
    plt.close(fig)
    ...
    data_save_path = '/home/links/ct715/eddy_feedback/chapter1/annular_modes/MSLP_NAO/data/pamip_misc'
