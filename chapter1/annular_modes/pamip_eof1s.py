...
def get_or_compute_eof(model, season, months, save_file, vertical_average=False):
    ...
    if os.path.isfile(save_file):
        ...
        if model == 'HadGEM3-GC31-LL':
            ds_model = ds_model.where(ds_model.level < 1000)
...
save_data_path = '/home/links/ct715/eddy_feedback/chapter1/annular_modes/data/eofs/pamip'
os.makedirs(save_data_path, exist_ok=True)
plot_base_path = '/home/links/ct715/eddy_feedback/chapter1/annular_modes/plots/pamip_eof1s'
...
for season, months in seasons.items():
    ...
    # -------------------------------------------------------------------------
    # Figure 1: Facet Grid of Full Resolution EOF Fields
    # -------------------------------------------------------------------------
    results = {}
    for model in models:
    ...
    for i, model in enumerate(models):
        ...
        eofs.sel(mode=0).plot.contourf(ax=ax, yincrease=False, levels=21)
        vf = results[model]['variance_fractions'][0].values * 100
        ax.set_title(f"{model} ({vf:.1f}%)")
    ...
    # -------------------------------------------------------------------------
    # Figure 2: Combined Line Plot of Vertically Averaged EOFs
    # -------------------------------------------------------------------------
    results2 = {}
    for model in models:
        file_name = f'{model}_eofs_va_{season}.nc'
        data_file = os.path.join(save_data_path, file_name)
        results2[model] = get_or_compute_eof(model, season, months, data_file, vertical_average=True)
    
    fig2, ax2 = plt.subplots(figsize=(11, 6))
    for model in models:
        eof_ds = results2[model]
        # Flip the EOF sign based on the test at lat=-60
    ...
    data_save_path = '/home/links/ct715/eddy_feedback/chapter1/annular_modes/data/pamip_misc'
