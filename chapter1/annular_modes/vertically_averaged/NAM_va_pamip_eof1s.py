...
def compute_anomalies(ds_model, season, months, vertical_average=True):
    """
    ...
    var_anoms = var_anoms.sel(lat=slice(0,90), level=slice(1000,100)).rename({'ens_ax': 'time'})
...
def get_or_compute_eof(model, season, months, save_file, vertical_average=True):
    """
...
save_data_path = '/home/links/ct715/eddy_feedback/chapter1/annular_modes/vertically_averaged/data/nh/pamip_NAM'
os.makedirs(save_data_path, exist_ok=True)
plot_base_path = '/home/links/ct715/eddy_feedback/chapter1/annular_modes/vertically_averaged/plots/nh/pamip_eof1s_NAM'
...
for season, months in seasons.items():
    ...
    # -------------------------------------------------------------------------
    # Figure 1: Facet Grid of Full Resolution EOF Fields (Polar Projection)
    # -------------------------------------------------------------------------
    results = {}
    for model in models:
        file_name = f'{model}_eofs_{season}.nc'
        data_file = os.path.join(save_data_path, file_name)
        results[model] = get_or_compute_eof(model, season, months, data_file, vertical_average=True)
    ...
    for i, model in enumerate(models):
        ...
        # Add coastlines and box
        ax.coastlines(linewidth=0.8, zorder=3)
        box_lons = [-80, -80, 30, 30, -80]
    ...
    fig.suptitle(f"EOF 1 of vertically-averaged U ({season})", fontsize=14)
    fig.tight_layout(rect=[0, 0.1, 1, 0.95])

    facet_plot_file = os.path.join(plot_save_path, f"facet_eof1_{season}.pdf")
    fig.savefig(facet_plot_file)
    print(f"Facet grid plot for {season} saved to {facet_plot_file}")
    plt.close(fig)
    ...
    data_save_path = '/home/links/ct715/eddy_feedback/chapter1/annular_modes/vertically_averaged/data/nh/pamip_misc'
