...
# ── EP-flux / daily-average phase ────────────────────────────────────────────

DAILY_AV_SAVE_DIR = '/gws/ssde/j25a/arctic_connect/cturrell/reanalysis_data/era5/6h_era5/daily_averages'
OMEGA_ROT = 2. * np.pi / 86400.
A0 = 6371000.


def process_year_epflux(year, save_dir=DAILY_AV_SAVE_DIR, force_recalculate=False):
    """Compute EP fluxes from 6-hourly ERA5 snapshots and save daily/monthly averages.

    EP fluxes are calculated on the 6-hourly data before any time averaging.
    Skips entirely if all three output files already exist (unless force_recalculate).
    """
    os.makedirs(save_dir, exist_ok=True)

    output_epflux_file  = os.path.join(save_dir, f'{year}_epflux.nc')
    output_day_av_file  = os.path.join(save_dir, f'{year}_daily_averages.nc')
    output_mon_av_file  = os.path.join(save_dir, f'{year}_monthly_averages.nc')

    all_exist = (
        os.path.isfile(output_epflux_file)
        and os.path.isfile(output_day_av_file)
        and os.path.isfile(output_mon_av_file)
    )
    if all_exist and not force_recalculate:
        logging.info(f'{year}: all output files already exist — skipping.')
        return

    logging.info(f'{year}: loading 6-hourly ERA5 snapshots for EP-flux calculation')

    rename_dims = {'latitude': 'lat', 'longitude': 'lon', 'level': 'pfull'}
    ds_u = xr.open_dataset(
        os.path.join(BASE_SAVE_DIR, 'u', f'{year}_u_6h_snapshots.nc'), chunks={'time': 10}
    ).rename({'u': 'ucomp', **rename_dims})
    ds_v = xr.open_dataset(
        os.path.join(BASE_SAVE_DIR, 'v', f'{year}_v_6h_snapshots.nc'), chunks={'time': 10}
    ).rename({'v': 'vcomp', **rename_dims})
    ds_t = xr.open_dataset(
        os.path.join(BASE_SAVE_DIR, 't', f'{year}_t_6h_snapshots.nc'), chunks={'time': 10}
    ).rename({'t': 'temp', **rename_dims})

    dataset = xr.merge([ds_u, ds_v, ds_t])

    logging.info(f'{year}: computing EP fluxes')
    epflux_ds = eff.ep_flux_calc(
        dataset, output_epflux_file,
        force_recalculate, False, OMEGA_ROT, A0
    )

    merged = xr.merge([dataset, epflux_ds])

    merged, duplicates_found = eff.check_for_duplicate_times(merged)
    if duplicates_found:
        merged, _ = eff.check_for_duplicate_times(merged)

    logging.info(f'{year}: computing daily and monthly averages')
    dataset_daily = eff.daily_average(
        merged, output_day_av_file, force_recalculate,
        monthly_too=True, monthly_output_file=output_mon_av_file
    )
    ...
    logging.info(f'{year}: done — files saved to {save_dir}')


if __name__ == '__main__':
    task_id = int(os.environ.get('SLURM_ARRAY_TASK_ID', sys.argv[1] if len(sys.argv) > 1 else 0))

    if task_id < len(VARIABLES):
        # Phase 1: extract and interpolate a single variable across all years
        ...
        # Phase 2: compute EP fluxes and averages for one year
        # task_id 3 → START_YEAR, task_id 4 → START_YEAR+1, …
        year = START_YEAR + (task_id - len(VARIABLES))
        if year > END_YEAR:
            raise ValueError(
                f'task_id {task_id} maps to year {year}, which exceeds END_YEAR={END_YEAR}'
            )
        logging.info(f"Computing EP fluxes for year {year} (task {task_id})")
        process_year_epflux(year)
