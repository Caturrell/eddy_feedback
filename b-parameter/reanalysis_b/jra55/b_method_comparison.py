...
plot_dir_b = '/home/links/ct715/eddy_feedback/b-parameter/reanalysis_b/jra55/b_plots/'
...
def plot_b_method_comparison(b_datasets, use_va):
    ...
    methods = METHOD_INFO if use_va is True else [m for m in METHOD_INFO if m[2] != 'partial_mean']
    ...
    fig.suptitle(
        f'b-parameter annual cycle — method comparison'
        + (f'  (va_str={va_str})' if va_str else ''),
        fontsize=13, y=1.01,
    )
    ...
    fname = f'b_method_comparison_NH_SH{va_str}.png'
    fpath = os.path.join(plot_dir_b, 'both_hemis' ,fname)
...
def plot_b_method_comparison_single_hem(b_datasets, use_va, hemisphere):
    ...
    fig.suptitle(
        f'b-parameter annual cycle — method comparison — {hem_label}'
        + (f'  (va_str={va_str})' if va_str else ''),
        fontsize=13, y=1.02,
    )
    ...
    fname = f'b_method_comparison_{hem_label}{va_str}.png'
    fpath = os.path.join(plot_dir_b, hem_label, fname)
    fig.savefig(fpath, dpi=300, bbox_inches='tight')
    print(f'Saved → {fpath}')
    plt.close(fig)
...
for use_va in [True, False, 500.]:
    plot_b_method_comparison(b_datasets, use_va)
    for hem in HEMISPHERES:
        plot_b_method_comparison_single_hem(b_datasets, use_va, hem)