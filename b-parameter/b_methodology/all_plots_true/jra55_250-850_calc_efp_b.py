"""
    Copied from: b-parameter/reanalysis_b/jra55/jra55_250-850_calc_efp_b.py
"""
...
# Shared output paths (do not vary by level config)
...
if level_type=='interp':
    ...
    include_udt_rdamp = 'udt_rdamp' in dataset.data_vars.keys()
else:
...
# Compare full pressure slice vs 3-level subset (pressure-weighted and simple mean)
level_configs = [
...
for pfull_slice_loop, level_subset_loop, pressure_weighted_loop, level_suffix in level_configs:
    ...
    output_eof_file = f'./{exp_name}_sit_plots/{start_month}_{end_month}/{level_type}/EOF{level_suffix}.nc'

    eof_plot_dir = f'{plot_dir}level{level_suffix}/'
    if not os.path.isdir(eof_plot_dir):
        os.makedirs(eof_plot_dir)

    eof_ds = eff.eof_calc(exp_type, output_eof_file, force_eof_recalculate, dataset, pfull_slice_loop,
                          subtract_annual_cycle, eof_vars, n_eofs, season_month_dict, anom_ds,
                          propogate_all_nans, level_subset=level_subset_loop,
